...
List boruvka (NumericMatrix graph, NumericMatrix adj, int comp){
    ...
    int start;//indice di partenza per dfs a contare il numero di nodi presenti nella componente
    List returned;
    NumericVector vis(v);
    for(i=0;i<v;i++){
    ...
    int x;
    if(v<e){
        x=e; //se il numero di archi è maggiore del numero di nodi allora le righe della tabella edge dovrà essere uguale al numero di archi
    }else{
        x=v;
    }
    NumericMatrix edge (x,3); //matrice contenente gli archi
    for(i=0;i<v;i++){
        for(j=0;j<v;j++){
            if(adj(i,j)==comp){
    ...
    NumericVector cheapest(v); //matrice valore minimo per giungere al nodo
    for(i=start;i<v;i++){
        subset(i,0)=i;//parente nodo i
        subset(i,1)=0;//rank nodo i
        cheapest(i)=-1;//arco con peso più basso per raggiungere un nodo da nodo partenza
    }
    int contarchi=nodi; //contatore archi nell'albero
